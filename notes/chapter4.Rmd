...
By plotting how genetic variance changes with gene frequency under different scenarios (__DEFINITELY DO THIS__). You see some interesting patterns. Firstly, at a single diallelic locus, you see that $\sigma^2_A$ reaches it's peak when $p_1 = p_2 = 0.5$. Secondly, it's clear that, even in the case of overdominance (which is rare!), additive genetic variance will almost always be much higher than genetic variance from dominance effects, even when the frequency of the dominant allele is high. (btw I think the scale on the top left graph is wrong...).
